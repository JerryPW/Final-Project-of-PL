...
struct SU_hash_cell {
  char * key;
  unsigned int value;
  struct SU_hash_cell * tail;
};
  
struct SU_hash_table {
  struct SU_hash_cell * (h[2048]);
};
...
struct SU_hash_table * init_SU_hash() {
  struct SU_hash_table * res = (struct SU_hash_table *) malloc(sizeof(struct SU_hash_table));
  if (res == NULL) {
    printf("Failure in malloc.\n");
    exit(0);
  }
  memset(res, 0, sizeof(struct SU_hash_table));
  return res;
}

unsigned int SU_hash_get(struct SU_hash_table * t, char * key) {
  unsigned int s = hash_fun(key);
  struct SU_hash_cell * p = t -> h[s];
  while (p != NULL) {
...
void SU_hash_set(struct SU_hash_table * t, char * key, unsigned int value) {
  ...
  * d = (struct SU_hash_cell *) malloc(sizeof (struct SU_hash_cell));
  if (* d == NULL) {
    printf("Failure in malloc.\n");
    exit(0);
  }
